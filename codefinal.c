...
glClearColor(0.05, 0.05, 0.05, 1.0);
	...
	// Summary Text
	print("India does not have any railways classified as high-speed rail (HSR),which have operational speeds in excess",
		1, 1, 1, summary_para1_fade, 55, 425, .06, .06, 0.5);
	print("of 200 km/h (120 mph).The fastest train in India is the Gatimaan Express with a top speed of ",1, 1, 1, summary_para1_fade, 55, 400, .06, .06, 1.25);
	print("160 km/h (99 mph) which runs between Delhi and Agra.",1, 1, 1, summary_para1_fade, 55, 375, .06, .06, 1.25);
		

	print("India's prime minister Narendra Modi approved the choice of Japan to build India's first high-speed railway.",
		1, 1, 1, summary_para2_fade, 55, 325, .06, .06, 1.25);
	print("India will use the wheel-based 300 km/hr HSR technology, instead of new maglev 600 km/hr technology of the Japan",
		1, 1, 1, summary_para2_fade, 55, 300, .06, .06 ,1.25);
print("used in Chuo Shinkansen.",
		1, 1, 1, summary_para2_fade, 55, 275, .06, .06 ,1.25);	
	print("Maglev is used for Bullet Train, Maglev is short for magnetic levitation, which means that these trains will float",
		1, 1, 1, summary_para3_fade, 55, 225, .06, .06, 1.25);
	print("over a guideway using the basic principles of magnets to replace the old steel wheel and track trains. ",
		1, 1, 1, summary_para3_fade, 55, 200, .06, .06, 1.25);

	print("The semi-high speed routes are Delhi-Chandigarh, Chennai-Bengaluru-Mysore, Delhi-Kanpur, Nagpur-Bilaspur, ",
		1, 1, 1, summary_para4_fade, 55, 125, .06, .06, 1.25);
	print("Mumbai-Ahmedabad, Chennai-Hyderabad and Nagpur-Secunderabd. It would be cost effective for common people.",
		1, 1, 1, summary_para4_fade, 55, 100, .06, .06, 1.25);
	print("This is a major impact for economy and will affect the future of India",
		1, 1, 1, summary_para4_fade, 55, 75, .06, .06, 1.25);

	print("Press K to move on...",
		1, 1, 1, summary_next_text_appear, 50, 10, .09, .09, 1);
}
...
Display_on_screen("DAYANANDA SAGAR COLLEGE OF ENGINEERING")	;
 ...
 setFont(GLUT_BITMAP_HELVETICA_18);
 drawstring(150.0,405.0,1.0,"Department of Computer Science and Engineering");    

 setFont(GLUT_BITMAP_HELVETICA_12); //Font set to helvetica with size 12
 glColor3f(1.0,1.0,0.0);
 drawstring(190.0,350.0,1.0,"A MINI PROJECT ON-");
 setFont(GLUT_BITMAP_HELVETICA_18);
drawstring(195.0,330.0,1.0,"BULLET TRAIN SIMULATION");
 ...
 drawstring(60.0,140.0,1.0,"RAKSHA M [1DS14CS128]");
drawstring(60.0,130.0,1.0,"ANINDITYA SAHA [1DS14CS129]");
drawstring(60.0,120.0,1.0,"AMITHA HM [1DS15CS402]");	
 
 setFont(GLUT_BITMAP_TIMES_ROMAN_24);
 glColor3f(1.0,1.0,1.0);
 drawstring(340.0,155.0,1.0,"Guided By-");
  setFont(GLUT_BITMAP_8_BY_13);
 drawstring(330.0,135.0,1.0,"Prof. Ravichandra H.");
 drawstring(330.0,125.0,1.0,"Prof. Kusuma M.");
 glFlush();
